...
const BadgeIcon = ({ name, iconName, difficulty, earned, size = 64 }) => {
  ...
  return (
    <div className={`relative flex items-center justify-center bdg-icon-wrapper bdg-${difficulty}`} style={{ width: size, height: size }}>
      ...
      <svg
        viewBox="0 0 100 100"
        className={`absolute inset-0 w-full h-full drop-shadow-2xl ${earned && difficulty === 'legendary' ? 'bdg-legendary-shield' : ''}`}
      ...
      >
        ...
        <path
          ...
          strokeDasharray="4 2"
        />
        {earned && difficulty === 'legendary' && (
           <path d="M50 5 L90 25 L50 95 Z" fill="rgba(255,255,255,0.08)" className="bdg-shine-path" />
        )}
      </svg>

      {/* Icon */}
      <div className={`relative z-10 ${earned && difficulty === 'rare' ? 'bdg-pulse-anim' : ''}`}>
      ...
      {earned && (
        <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1 h-2 bdg-accent-anim" style={{ backgroundColor: colors.primary }} />
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-2 bdg-accent-anim" style={{ backgroundColor: colors.primary }} />
        </div>
      )}
    </div>
  );

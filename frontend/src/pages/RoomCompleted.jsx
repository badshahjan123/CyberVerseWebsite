...
const RoomCompleted = () => {
  ...
  const stats = {
    ...
    xp:
      ...
      progress.taskScores?.reduce(
        (sum, task) => sum + (task.pointsEarned || 0),
        0
      ) + (progress.quizScore?.pointsEarned || 0) ||
      room.points ||
      500,
    timeSpent: "15 min",
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-slate-300">
            ...
            <span className="text-primary font-semibold">{room.title}</span>
          </p>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700 hover:border-primary/30 transition-colors">
              ...
              <p className="text-sm text-slate-400 mb-3">
                Continue with {room.category} challenges
              </p>
              <Link
                to={`/rooms?category=${room.category}`}
            ...
            <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700 hover:border-accent/30 transition-colors">
              ...
              <Link
                to="/rooms?difficulty=Hard"
                className="text-accent hover:text-accent-hover text-sm font-semibold flex items-center gap-1"
              >
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link
            to={`/rooms/${roomId}`}
            className="btn-ghost px-6 py-3 text-lg flex items-center justify-center gap-2"
          >
          ...
          >
            <RotateCcw
              className={`h-5 w-5 ${resetting ? "animate-spin" : ""}`}
            />

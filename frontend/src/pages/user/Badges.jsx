    ...
    >
        ...
        <h3 className="bdg-card__name">
          {badge.earned ? (
            ...
            <>
              <Lock
                size={12}
                style={{ display: "inline", marginRight: 5, opacity: 0.4 }}
              />
              Locked
            </>
          )}
      ...
      <div className="bdg-card__footer">
        {badge.earned ? (
          ...
          <div className="bdg-card__unlock-hint">{badge.unlockReason}</div>
        )}
      </div>
    </div>
  );
...
const Badges = () => {
    ...
    switch (tab) {
      ...
      case "milestone":
        return badges.filter(
          (b) => b.category === "milestone" || b.badgeType === "milestone",
        );
      case "skill":
        return badges.filter(
          (b) => b.category === "skill" || b.badgeType === "skill",
        );
      case "streak":
        return badges.filter(
          (b) => b.category === "streak" || b.badgeType === "streak",
        );
      case "special":
        return badges.filter(
          (b) => b.category === "special" || b.badgeType === "special",
        );
      default:
        return badges;
    }
  ...
  const bonusEarned = badges.filter(
    (b) => b.earned && b.badgeType === "bonus",
  ).length;
      ...
      <div className="bdg-wrap">
        ...
        <header className="bdg-header">
          <div className="bdg-header__left">
            <div className="bdg-header__icon">
              <Trophy size={20} />
            </div>
            <div>
              <h1 className="bdg-header__title">Badges</h1>
              <p className="bdg-header__sub">
                Verified achievements earned through field operations in
                CyberVerse.
              </p>
            </div>
          </div>
          ...
          <div className="bdg-stats">
            <div className="bdg-stat">
              <div className="bdg-stat__val" style={{ color: "#00F5FF" }}>
                {earned}
                <span className="bdg-stat__total">/{total}</span>
              </div>
              <div className="bdg-stat__label">Earned</div>
            </div>
            <div className="bdg-stat-divider" />
            <div className="bdg-stat">
              <div className="bdg-stat__val" style={{ color: "#FACC15" }}>
                {bonusEarned}
              </div>
            ...
            <div className="bdg-stat-divider" />
            <div className="bdg-stat">
              <div className="bdg-stat__val" style={{ color: "#39FF14" }}>
            ...
            <section className="bdg-section">
              {tab === "all" && (
                <div className="bdg-section__header">
                  ...
                  <span>
                    Earned — {filtered.filter((b) => b.earned).length}
                  </span>
                </div>
              )}
            ...
            <section className="bdg-section">
              {tab === "all" && (
                <div className="bdg-section__header bdg-section__header--locked">
                  ...
                  <span>
                    Locked — {filtered.filter((b) => !b.earned).length}
                  </span>
                </div>
              )}
              <div className="bdg-grid">

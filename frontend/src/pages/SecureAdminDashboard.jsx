...
if (typeof globalThis !== "undefined") {
  globalThis.DISABLE_WEBSOCKET = true;
}
...
const SecureAdminDashboard = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRecentActivity([
          ...
          {
            type: "room",
            message: "Room completed: SQL Injection Challenge",
            timestamp: "5 minutes ago",
          },
          {
            type: "lab",
            message: "New lab published: Network Security",
  ...
  const deleteLab = async (labId) => {
    if (globalThis.confirm("Are you sure you want to delete this lab?")) {
      try {
        const token = localStorage.getItem("token");
    ...
    try {
      ...
      const url =
        modalType === "room"
          ...
          : "http://localhost:5000/api/admin/labs";
  ...
  const bulkDeleteUsers = async () => {
    ...
    if (globalThis.confirm(`Delete ${selectedUsers.length} selected users?`)) {
      try {
        const token = localStorage.getItem("token");
  ...
  // Helper to centralize difficulty badge classes (avoids nested ternaries)
        ...
        <main className="flex-1 p-6">
          <div className="mb-8">
            ...
            <p className="text-gray-400">
              ...
              {activeTab === "rooms" &&
                "Create and manage cybersecurity challenge rooms"}
              {activeTab === "labs" &&
                "Organize and maintain hands-on learning laboratories"}
            </p>
          </div>
                  ...
                  <div className="space-y-4 max-h-64 overflow-y-auto">
                    {recentActivity.length > 0 ? (
                      recentActivity.map((activity, index) => (
                        <div
                          key={`${activity.timestamp}-${activity.message}-${index}`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {labs
                  ...
                  .map((lab) => (
                    <div
                      key={lab._id}
                      className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg hover:shadow-purple-500/20 hover:border-purple-500/50 transition-all group"
                    >
                      <div className="flex justify-between items-start mb-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  className="rounded border-gray-600 bg-gray-700 text-cyan-400 focus:ring-cyan-400"
                />
                <label htmlFor="modal-premium" className="text-gray-300">
                  Premium Content
                </label>
              </div>
